...
**Note**: In defiNNe V_0-1-0, this field was named `parent`. Starting with defiNNe V_0-1-1, the field is `parent_spec`.
...
### 7.1. Migration From V_0-1-0

**What changed**: The frontmatter field `parent` has been renamed to `parent_spec`.

**Why**: The term `parent` is ambiguous — it could refer to a tree/graph parent (taxonomy hierarchy) or a specification dependency. `parent_spec` clearly identifies the field as a specification-level parent reference used for the dependency resolution chain.

**How to migrate**:

For any model, template, or specification frontmatter that declares `parent:`:
...
1. Replace `parent:` with `parent_spec:` in the YAML frontmatter key.
...
The following specification versions are superseded by newer releases:
...
From the sample model `specs/business_V_0-1-1/samples/Ghostbusters_V_0-1-2_business_F.md`:

```yaml
# Ghostbusters (level 3) — `samples/business/Ghostbusters_V_0-1-2_business_F.md`
parent_spec:
...
# defiNNe_V_0-1-1 (level 0) — this document
...
📁 Ghostbusters_V_0-1-0_business/
  📄 Ghostbusters_V_0-1-0_business_F.md
  📁 specs/

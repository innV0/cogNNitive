...
- A level N specification (N > 0) MUST declare `parent` as an object with `name` and `url`, pointing to its level N-1 parent.
...
The `parent` field is an object, not a string:
...
3. Save it to `specs/<parent.name>_FORMAT.md`.
4. Read the downloaded spec's `parent.url` and repeat until reaching level 0 (no parent).
5. On subsequent loads, check `specs/` first. Only download missing files.
6. If the model's `parent` changes (version bump), the application detects the mismatch and downloads the new parent.

The cached directory structure:

```
📁 <Model>_V_x-y-z_<Template>/
  📄 <Model>_V_x-y-z_<Template>_FORMAT.md
  📁 specs/
    📄 <parent.name>_FORMAT.md        ← level 2 (template)
    📄 <grandparent.name>_FORMAT.md   ← level 1 (FORMAT or iNNfo)
    📄 <great-grandparent.name>_FORMAT.md  ← level 0 (defiNNe)
```
...
A level 3 model MUST NOT inline the template definition. It MUST rely on the `parent` URL + spec resolver to obtain its schema.
...
5. If level > 0: frontmatter contains `parent` as an object with `name` and `url`.

...
| Spec | Version | Source |
|------|---------|--------|
| **FORMAT** | V 0.1.0 | [`specs/FORMAT_V_0-1-0_F.md`](https://github.com/innV0/cogNNitive/blob/main/specs/FORMAT_V_0-1-0_F.md) |
| **FORMAT** | V 0.1.1 | [`specs/FORMAT_V_0-1-1_F.md`](https://github.com/innV0/cogNNitive/blob/main/specs/FORMAT_V_0-1-1_F.md) |
...
|-------|---------|----------|--------|
| **Ghostbusters** | V 0.1.1 | business | [`models/Ghostbusters_V_0-1-1_business_F.md`](https://github.com/innV0/cogNNitive/blob/main/models/Ghostbusters_V_0-1-1_business_F.md) |
...
|---|---|
| Parseable YAML frontmatter on every non-reserved `.md` file | ✅ Satisfied — every `_F.md` has required frontmatter |
...
2. **OKF tolerates extensions**: OKF explicitly allows unknown frontmatter keys and unknown `type` values. FORMAT's additional fields (`spec_version`, `level`, `parent`, `concepts`, `markers`, `matrices`) are fully tolerated.
3. **FOLDER mode = OKF Bundle**: FORMAT FOLDER mode produces the exact directory-of-Markdown-files structure OKF defines as a knowledge bundle (§3). Each `_F.md` is an OKF concept document (§4) with `index.md` as the directory listing (§6).

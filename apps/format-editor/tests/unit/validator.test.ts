...
describe('validateFormatContent (ported from launcher)', () => {
  it('returns all expected check IDs', () => {
    const report = validateFormatContent(validModel, 'Ghostbusters_V_0-1-2_business_FORMAT.md', 'FILE')
  ...
  it('passes frontmatter checks for valid model', () => {
    const report = validateFormatContent(validModel, 'test_FORMAT.md', 'FILE')
  ...
  it('detects missing frontmatter fields', () => {
    ...
    const report = validateFormatContent(noTitle, 'test_FORMAT.md', 'FILE')
  ...
  it('detects invalid version format', () => {
    ...
    const report = validateFormatContent(badVersion, 'test_FORMAT.md', 'FILE')
  ...
  it('flags missing _F element markers', () => {
    ...
    const report = validateFormatContent(noMarkers, 'test_FORMAT.md', 'FILE')
  ...
  it('works without explicit mode (infers from frontmatter)', () => {
    const report = validateFormatContent(validModel, 'test_FORMAT.md')
    expect(report.mode).toBe('FILE')
  })

  it('works for FOLDER mode', () => {
    const folderModel = validModel.replace(/^mode:.*$/m, 'mode: "FOLDER"')
    const report = validateFormatContent(folderModel, '_FORMAT.md', 'FOLDER')

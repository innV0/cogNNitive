<template>
  <div class="compliance-tab">
    ...
    >
      <span>
        ...
        <template v-if="errorCount > 0">
          , <strong class="text-red-600 dark:text-red-400">{{ errorCount }} error{{ errorCount !== 1 ? 's' : '' }}</strong>
        </template>
        <template v-if="warningCount > 0">
          , <strong class="text-amber-600 dark:text-amber-400">{{ warningCount }} warning{{ warningCount !== 1 ? 's' : '' }}</strong>
        </template>
      </span>
      <span v-if="passedCount === totalCount && totalCount > 0" class="text-xs font-semibold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">
        All good
      </span>
    </div>
    ...
    <div v-if="scopedChecks.length > 0" class="space-y-1">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          <span class="font-medium" :class="check.passed ? 'text-slate-600 dark:text-slate-300' : 'text-slate-800 dark:text-slate-200'">
            {{ check.label }}
          </span>
          <p v-if="!check.passed && check.message" class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 font-mono">
            {{ check.message }}
          </p>
        </div>
      </div>
    ...
    <!-- Empty state -->
    <p v-if="scopedChecks.length === 0" class="text-xs text-slate-400 dark:text-slate-500 italic px-3 py-2">
...
const errorCount = computed(() => scopedChecks.value.filter(c => !c.passed && c.severity === 'error').length)
const warningCount = computed(() => scopedChecks.value.filter(c => !c.passed && c.severity === 'warning').length)

const summaryClass = computed(() => {
  if (errorCount.value > 0) return 'bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-200 border border-red-200 dark:border-red-800'
  if (warningCount.value > 0) return 'bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-200 border border-amber-200 dark:border-amber-800'
  if (totalCount.value > 0) return 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-800 dark:text-emerald-200 border border-emerald-200 dark:border-emerald-800'

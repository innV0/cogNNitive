<template>
  ...
  >
    ...
    <span v-if="showSourceTarget && source && target" class="truncate min-w-0 leading-tight flex items-center gap-1">
      ...
      <span v-if="label" class="text-xs text-slate-400 dark:text-slate-500 font-normal italic">{{ label }}</span>
...
const sourceAccent = computed(() => sourcePalette.value?.accent || 'text-slate-400 dark:text-slate-500');

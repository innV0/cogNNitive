<template>
  ...
  >
    <div class="flex items-center gap-4">
      ...
      <div
        v-if="hasRootNode"
        class="flex items-center gap-3 pl-3 border-l border-slate-200 dark:border-slate-600 text-xs text-slate-500 dark:text-slate-400"
      >
        <!-- Format Version -->
        ...
        <!-- Status Badge -->
        <div
          class="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-2xs font-bold ring-1 ring-inset"
          :class="
            unsavedChanges
        ...
        <div class="flex items-center gap-1">
          ...
          <button
            @click="copyFilePath"
            class="p-0.5 rounded text-slate-400 hover:text-primary hover:bg-primary/5 transition-colors cursor-pointer mr-2"
            title="Copy path"
          >
            <Copy class="w-3 h-3" />
          </button>
        ...
        <div class="flex items-center gap-1 border-l border-slate-200 dark:border-slate-600 pl-3">
          <button
            @click="openValidationReport"
            class="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-2xs font-bold ring-1 ring-inset transition-all cursor-pointer"
            :class="statusClass"
            :title="statusTitle"
          >
        ...
        <div class="relative inline-flex rounded-md shadow-xs">
          ...
          >
            <div class="px-3 pt-1">
              ...
              <p
                v-if="bumpError"
                class="text-2xs text-red-600 dark:text-red-400 mb-1.5 leading-tight"
              >
                {{ bumpError }}
              </p>
...
import { Copy, Save, ChevronDown, Info, CheckCircle, AlertTriangle, XCircle } from 'lucide-vue-next'
...
const statusClass = computed(() => {
  if (hasErrors.value) {
    return 'bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 ring-red-600/20 hover:bg-red-100 dark:hover:bg-red-900/50'
  }
  if (hasWarnings.value) {
    return 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 ring-amber-600/20 hover:bg-amber-100 dark:hover:bg-amber-900/50'
  }
  return 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 ring-emerald-600/20 hover:bg-emerald-100 dark:hover:bg-emerald-900/50'
})
...
const statusText = computed(() => {
  if (hasErrors.value) return `Issues (${totalErrors.value})`
  if (hasWarnings.value) return `Attention (${totalWarnings.value})`
  return 'OK'
})

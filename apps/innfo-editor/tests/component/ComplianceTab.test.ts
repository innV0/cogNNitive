...
function makeReport(checks: Array<{
  id: string
  label: string
  passed: boolean
...
describe('ComplianceTab.vue — R-SC-07 (Compliance tab)', () => {
  ...
  it('filters checks to those scoped to the concept type by label', () => {
    const report = makeReport([
      { id: 'name-check', label: 'Task description required', passed: false, severity: 'error', message: 'Missing description', category: 'task' },
  ...
  it('includes frontmatter checks for all concept types', () => {
    const report = makeReport([
      { id: 'frontmatter-version', label: 'Spec version present', passed: true, category: 'frontmatter' },
      { id: 'task-name', label: 'Task name', passed: true, category: 'task' },
    ])
  ...
  it('shows error count and styling when checks fail', () => {
    const report = makeReport([
      { id: 'task-name', label: 'Task name required', passed: false, severity: 'error', message: 'Name is empty', category: 'task' },
  ...
  it('shows warning count when warnings exist', () => {
    const report = makeReport([
      { id: 'task-name', label: 'Task name', passed: false, severity: 'warning', message: 'Consider a more descriptive name', category: 'task' },
      { id: 'task-status', label: 'Task status', passed: true, category: 'task' },
    ])
